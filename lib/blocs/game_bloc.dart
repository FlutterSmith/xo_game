...
class GameBloc extends Bloc<GameEvent, GameState> {
  ...
  FutureOr<void> _onMoveMade(MoveMade event, Emitter<GameState> emit) async {
    ...
    final result = checkWinnerDynamic(newBoard, state.boardSize, state.winCondition);
    if (result != null) {
      final newHistory = List<String>.from(state.gameHistory);
      String outcome = result['winner'] == 'Draw' ? "Draw" : "Winner: ${result['winner']}";
      newHistory.add(outcome);
      await DatabaseHelper.instance.insertHistory(outcome);
      add(const LoadHistory());
      emit(state.copyWith(board: newBoard, gameOver: true, resultMessage: outcome, winningCells: result['winningCells'], currentPlayer: state.currentPlayer, undoStack: newUndoStack, redoStack: [], gameHistory: newHistory));
      return;
    } else {
  ...
  FutureOr<void> _onResetGame(ResetGame event, Emitter<GameState> emit) {
    emit(GameState.initial().copyWith(gameMode: state.gameMode, aiDifficulty: state.aiDifficulty, gameHistory: state.gameHistory, boardSize: state.boardSize, winCondition: state.winCondition));
    return null;
  }
  FutureOr<void> _onToggleGameMode(ToggleGameMode event, Emitter<GameState> emit) {
    ...
    emit(GameState.initial().copyWith(gameMode: newMode, aiDifficulty: state.aiDifficulty, gameHistory: state.gameHistory, boardSize: state.boardSize, winCondition: state.winCondition));
    return null;
  }
  FutureOr<void> _onChangeDifficulty(ChangeDifficulty event, Emitter<GameState> emit) {
    emit(GameState.initial().copyWith(gameMode: state.gameMode, aiDifficulty: event.difficulty, gameHistory: state.gameHistory, boardSize: state.boardSize, winCondition: state.winCondition));
    return null;
  }
  FutureOr<void> _onLoadHistory(LoadHistory event, Emitter<GameState> emit) async {
    final history = await DatabaseHelper.instance.getHistory();
    emit(state.copyWith(gameHistory: history));
    return;
  }
  FutureOr<void> _onUpdateBoardSettings(UpdateBoardSettings event, Emitter<GameState> emit) {
    emit(GameState.initial().copyWith(gameMode: state.gameMode, aiDifficulty: state.aiDifficulty, gameHistory: state.gameHistory, boardSize: event.boardSize, winCondition: event.winCondition));
    return null;
  }
  ...
  int _minimax(List<String> board, int depth, bool isMaximizing, String aiPlayer, String humanPlayer, int depthLimit) {
    final result = checkWinnerDynamic(board, state.boardSize, state.winCondition);
